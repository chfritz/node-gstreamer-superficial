	  ...
	  "conditions" : [
		["OS=='linux'", {
			"include_dirs": [
				...
				'<!@(env PKG_CONFIG_SYSROOT_DIR=/home pkg-config gstreamer-1.0 --cflags-only-I | sed s/-I//g)',
				'<!@(pkg-config gstreamer-app-1.0 --cflags-only-I | sed s/-I//g)',
				'<!@(env PKG_CONFIG_SYSROOT_DIR=/home pkg-config gstreamer-app-1.0 --cflags-only-I | sed s/-I//g)',
